...
from app.ai.sps_v3_engine.freshness import Freshness, FreshnessClass, evaluate_freshness, freshness_class_for
from app.ai.sps_v3_engine.registry import ParameterRegistry
from app.ai.sps_v3_engine.signals import CanonicalSignal, build_canonical_signals
...
def apply_staleness_filter(
    company: EvidenceBundle,
    reference_date: date | None,
    registry: ParameterRegistry,
) -> tuple[EvidenceBundle, tuple]:
    ...
    filtered = EvidenceBundle(company.company_id, company.stage, tuple(kept), company.negative_signals)
...
def eval_market_size(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_market_growth(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_market_timing(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_competitive_intensity(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_customer_demand(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_founder_market_fit(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_technical_capability(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_business_capability(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_leadership(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_execution_track_record_team(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_customer_value(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = company.evidence_of_type(CustomerEvidenceObservation)
...
def eval_differentiation(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_product_accessibility(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_defensibility(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_adoption_potential(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_gtm_execution(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_product_execution(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_operating_discipline(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_strategic_execution(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    obs = tuple(
        e for e in company.evidence
...
def eval_current_scale(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_growth_trajectory(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
    revenue_raw = tuple(e for e in company.evidence if isinstance(e, RevenueObservation) and e.metric_type == RevenueMetricType.ARR)
...
def eval_customer_adoption(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_retention_engagement(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_commercial_validation(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_revenue_quality(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_unit_economics(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
def eval_capital_efficiency(company: EvidenceBundle, registry: ParameterRegistry) -> DimensionResult:
...
ALL_EVALUATORS: dict[str, Callable[[EvidenceBundle, ParameterRegistry], DimensionResult]] = {
